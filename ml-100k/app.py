...
def load_data(data_path="ml-100k/u.data", item_path="ml-100k/u.item"):
    ...
    item_cols = ["item_id", "title"] + [f"g{i}" for i in range(19)]
...
class SVDRecommender:
    ...
    def recommend(self, user_idx, R_train, items_df, user_id_map, item_id_map, n=10):
        ...
        for item_id, score in top_n:
            title = items_df[items_df.item_id == item_id]["title"].values
            result.append({
                "Movie": title[0] if len(title) > 0 else f"Item {item_id}",
...
with tab1:
    ...
    try:
        ...
        fig, ax = plt.subplots(figsize=(12, 4), facecolor="#13131f")
        ax.set_facecolor("#13131f")
        mask = sample.isna()
        ...
        ax.set_xlabel("Movie ID", color="#cdd6f4")
        ax.set_ylabel("User ID", color="#cdd6f4")
        ax.tick_params(colors="#cdd6f4")
        ax.set_title("⬛ = Missing (NaN)", color="#cdd6f4", pad=6)
        plt.tight_layout()
        st.pyplot(fig)
        plt.close()
...
with tab4:
    ...
    if "model" in st.session_state:
        ...
        with col1:
            ...
            rec_btn = st.button("Get Recommendations", type="primary")

        if rec_btn:
            u_idx = user_ids.index(selected_user)
            recs = model.recommend(u_idx, R_train, items, user_ids, item_ids, n=n_recs)

            with col2:
                st.markdown(f"**Top {n_recs} Recommendations for User {selected_user}**")
                fig, ax = plt.subplots(figsize=(7, n_recs * 0.38 + 0.5), facecolor="#13131f")
                ax.set_facecolor("#13131f")
                colors = plt.cm.plasma(np.linspace(0.3, 0.85, len(recs)))
                bars = ax.barh(recs["Movie"], recs["Predicted Rating"], color=colors)
                ax.set_xlim(1, 5.5)
                ax.axvline(4, color="#3ECFCF", linestyle="--", alpha=0.5, linewidth=1)
                ax.set_xlabel("Predicted Rating", color="#cdd6f4")
            ...
            # Also show already-rated movies
            rated_mask = ~np.isnan(R_train[u_idx])
            rated_items = np.where(rated_mask)[0]
            rated_df = pd.DataFrame({
                "Movie": [items[items.item_id == item_ids[i]]["title"].values[0]
                          if len(items[items.item_id == item_ids[i]]) > 0 else f"Item {item_ids[i]}"
                          for i in rated_items[:15]],
                "Actual Rating": [R_train[u_idx, i] for i in rated_items[:15]]
            }).sort_values("Actual Rating", ascending=False)

            st.markdown(f"**Movies Already Rated by User {selected_user}** (first 15)")
